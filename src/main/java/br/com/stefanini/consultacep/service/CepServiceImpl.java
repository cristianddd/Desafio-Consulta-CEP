...
public class CepServiceImpl implements CepService {
    ...
    @Override
    public EnderecoResponseDTO consultarPorCep(String cep) {
        logger.info("Iniciando consulta para CEP: {}", cep);
        EnderecoResponseDTO response = cepClient.buscarCep(cep);
    ...
    private void salvarLog(EnderecoResponseDTO response) {
        ...
        try {
            ...
            logger.error("Erro ao tentar salvar no DynamoDB", ex);
            throw new DynamoDbErroException(HttpStatus.INTERNAL_SERVER_ERROR, "Error ao tentar salvar no banco.");
        }
    }
}
